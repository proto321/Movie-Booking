...
import { Calendar, Film, List, Menu, Ticket, X as XIcon } from 'lucide-react';
...
const Navbar = () => {
  ...
  }, [open, close]);// this function helps in close the menufor moble view when click on escape btn 
  ...
  const NavItem = ({ to, Icon, label, end = false, onClick }) => (
    ...
    >
      {({ isActive }) => (
        <>
          <Icon
            className={`${styles4.navLinkIconBase} ${isActive ? styles4.navLinkIconActive : styles4.navLinkIconInactive
              }`}
          />
          <span
            className={`${styles4.navLinkTextBase} ${isActive ? styles4.navLinkTextActive : styles4.navLinkTextInactive
              }`}
          >
            {label}
          </span>
  ...
  return (
    <nav className={styles4.navbar}>
      <div className={styles4.navbarContainer}>
        <div className={styles4.navbarFlex}>
          ...
          <div className={styles4.logoContainer}>
            ...
            <span className={styles4.logoText}>MOVIEMANIA</span>
          </div>
      ...
      <div
        className={`${styles4.mobileMenuContainer} ${open ? "pointer-events-auto" : "pointer-events-none"
          }`}
      >
        <div
          className={`${styles4.mobileMenuBackdrop} ${open ? 'opacity-100' : 'opacity-0'
            }`} onClick={close}
        />

        <aside id='mobile-menu' className={`${styles4.mobileMenuPanel} ${open ? 'translate-x-0' : 'translate-x-full'
          }`} role='dialog'>
          <div className={styles4.mobileMenuPanelHeader}>
            <div className={styles4.logoContainer}>
              <div className='flex items-center justify-center w-10 h-10 bg-blue-600 rounded-full transform rotate-12'>
            ...
            <button onClick={close} className='p-2 rounded-full hover:bg-white/5 focus:outline-none focus:ring-2
            focus:ring-blue-600' >
              <XIcon className='w-5 h-5 text-blue-200' />
            </button>
          </div>
          ...
          <nav className={styles4.mobileMenuPanelNav}>
            <NavLink
              ...
              onClick={close}
              className={({ isActive }) =>
                `${styles4.mobileNavLinkBase} ${isActive
          ...
          <div className={styles4.mobileMenuPanelFooter}>
            <p className={styles4.mobileMenuFooterText}>
                &copy; {new Date().getFullYear()} MOVIEMANIA
            </p>
          </div>
        </aside>
      </div>
